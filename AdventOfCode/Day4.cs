...
namespace AdventOfCode
{
    internal static class Day4
    {
        ...
        internal static int GetPart2Result()
        {
            var numberList = bingoBoards.Trim().Split(null).Where(x => !string.IsNullOrWhiteSpace(x)).Select(x => int.Parse(x));
            IEnumerable<int> tempList = null;

            Dictionary<int, int> winningBoards = new Dictionary<int, int>();
            List<int> existingWinners = new List<int>();
            var counter = 0;
            foreach (var bingoNumber in bingoNumbers)
            {
                tempList = tempList?.Select(x => x == bingoNumber ? -1 : x) ?? numberList.Select(x => x == bingoNumber ? -1 : x);
                var isColumn = IsColumn(tempList, existingWinners);
                var isRow = IsRow(tempList, existingWinners);

                foreach (var winner in existingWinners)
                {
                    if (winningBoards.ContainsKey(winner)) continue;

                    var winningBoard = tempList.Skip(winner).Take(25);
                    var unmarkedSum = winningBoard.Where(x => x != -1).Sum();
                    winningBoards[winner] = bingoNumber * unmarkedSum;
                    counter++;
                }
            }
        ...
        private static IEnumerable<int> IsRow(IEnumerable<int> inp, List<int> indexToIgnore)
        {
            var counter = 0;
            var startIndex = 0;
            ...
            {
                var r = inp.Skip(counter).Take(5).Sum() == -5;
                if (r && !indexToIgnore.Contains(startIndex))
                {
                    indexToIgnore.Add(startIndex);
                };
                counter += 5;
                if (counter % 25 == 0) startIndex += 25;
                if (counter == inp.Count()) return indexToIgnore;
            }

            return indexToIgnore;
        }

        private static IEnumerable<int> IsColumn(IEnumerable<int> inp, List<int> indexToIgnore)
        {
            var counter = 0;

            foreach (var n in inp)
            {
                var r = inp.Skip(counter).Take(25).ToArray();

                for (int i = 0; i < 5; i++)
                {
                    if (r.Where((x, index) => index == i * 5).Sum() == -5 && !indexToIgnore.Contains(counter))
                    {
                        indexToIgnore.Add(counter);
                    }
                }
        ...
        private static int? IsRow(IEnumerable<int> inp)
        {
            var counter = 0;
            var startIndex = 0;
            ...
            {
                var r = inp.Skip(counter).Take(5).Sum() == -5;
                if (r) return startIndex;
                counter += 5;
                if (counter % 25 == 0) startIndex += 25;
                if (counter == inp.Count()) return null;
            }

            return null;
        }

        private static int? IsColumn(IEnumerable<int> inp)
        {
            var counter = 0;
            ...
            foreach (var n in inp)
            {
                ...
                for (int i = 0; i < 5; i++)
                {
                    if (r.Where((x, index) => index == i * 5).Sum() == -5)
                    {
                        return counter;
                    }
                }
